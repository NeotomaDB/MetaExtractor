...
: Labelled Sampled Article {#tbl-relevance_sample}
...
: Sentence Embedding Models {#tbl-sent_embedding}
...
def plot_distribution_of_entities(
    ...
    ax = sns.barplot(x="entity", y="count", data=flat_df, ax=ax)
...
plot_distribution_of_entities(
    test_results_df, "Spacy Transformer V3", title="Entity Distribution for Test Set"
)
...
|                  |                                                                      | -   Slow processing                                  |
+------------------+----------------------------------------------------------------------+------------------------------------------------------+
| en_core          | -   A smaller word vector model with static embeddings for words     | -   Static embeddings without context                |
| _web_md          |                                                                      |                                                      |
...
From the Message Understanding conference [@chinchor-sundheim-1993-muc] the following detailed metrics are used during evaluation for each of the above methods:
...
@fig-extraction_recall_results shows the primary candidate models token and entity based recall.

```{python}
#| label: fig-extraction_recall_results
