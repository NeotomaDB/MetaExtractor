...
The state of the art for NER tasks is currently dominated by transformer based models. The best performing non-transformer based models are a transition-based stack long-short term memory (S-LSTM) [@S-LSTM] model used in the spaCy package.
...
The end goal of this project is to have each data product running in a semi/un-supervised fashion. The article relevance prediction pipeline is containerized using Docker[@docker]. It is expected to run on a daily daily or weekly basis by the Neotoma and gets the latest published articles from the public xDD API [@xDD], runs the article relevance prediction, and finally submits relevant articles to xDD to have their full text processed. 
